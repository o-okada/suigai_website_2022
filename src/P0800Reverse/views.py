    ...
    try:
        ...
        if ken_code == "0" and city_code == "0":
            ...
            ippan_reverse_list = IPPAN_VIEW.objects.raw("""
                ...
                FROM IPPAN_VIEW IV1 
                WHERE IV1.city_code=%s 
                ORDER BY CAST(IV1.ippan_id AS INTEGER)""", [city_code, ])
            ...
            ippan_reverse_list = IPPAN_VIEW.objects.raw("""
                ...
                FROM IPPAN_VIEW IV1 
                WHERE IV1.ken_code=%s 
                ORDER BY CAST(IV1.ippan_id AS INTEGER)""", [ken_code, ])
...
def summary_ken_city_category_view(request, ken_code, city_code, category_code):
    try:
        ...
        print_log('[INFO] P0800Reverse.summary_ken_city_category_view()関数 STEP 1/6.', 'INFO')
        ...
        ### (8)事業所応急対策費 = 事業所数 x 代替活動費
        #######################################################################
        print_log('[INFO] P0800Reverse.summary_ken_city_category_view()関数 STEP 2/6.', 'INFO')
 
        ####################################################################### 
        ### 延床面積(入力DB)から逆計算により被害建物棟数を求めた結果
        ####################################################################### 
        print_log('[INFO] P0800Reverse.summary_ken_city_category_view()関数 STEP 4/6.', 'INFO')
        ippan_reverse_list = None
        ippan_reverse_list = IPPAN_VIEW.objects.raw("""
            SELECT 
                IV1.ippan_id AS ippan_id, 
        ...
        ### テンプレートとコンテキストを設定して、レスポンスをブラウザに戻す。
        #######################################################################
        print_log('[INFO] P0800Reverse.summary_ken_city_category_view()関数 STEP 6/6.', 'INFO')
        ### template = loader.get_template('P0800Reverse/index.html')
        template = loader.get_template('P0800Reverse/summary.html')
        context = {
            'category_code': category_code, 
