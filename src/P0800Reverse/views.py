...
def input_category_view(request, category_code):
    try:
        ...
        #######################################################################
        print_log('[INFO] ########################################', 'INFO')
        print_log('[INFO] P0800Reverse.input_category_view()関数が開始しました。', 'INFO')
        print_log('[INFO] P0800Reverse.input_category_view()関数 request = {}'.format(request.method), 'INFO')
        print_log('[INFO] P0800Reverse.input_category_view()関数 category_code = {}'.format(category_code), 'INFO')
        print_log('[INFO] P0800Reverse.input_category_view()関数 STEP 1/6.', 'INFO')
        ...
        #######################################################################
        print_log('[INFO] P0800Reverse.input_category_view()関数 STEP 2/6.', 'INFO')
 
        ####################################################################### 
        ### 延床面積(入力DB)から逆計算により被害建物棟数を求めた結果
        ####################################################################### 
        print_log('[INFO] P0800Reverse.input_category_view()関数 STEP 4/6.', 'INFO')
        ...
        ### テンプレートとコンテキストを設定して、レスポンスをブラウザに戻す。
        #######################################################################
        print_log('[INFO] P0800Reverse.input_category_view()関数 STEP 6/6.', 'INFO')
        ### template = loader.get_template('P0800Reverse/index.html')
        template = loader.get_template('P0800Reverse/input.html')
        context = {
        ...
        print_log('[INFO] P0800Reverse.input_category_view()関数が正常終了しました。', 'INFO')
        return HttpResponse(template.render(context, request))
    
    except:
        print_log(sys.exc_info()[0], 'ERROR')
        print_log('[ERROR] P0800Reverse.input_category_view()関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P0800Reverse.input_category_view()関数が異常終了しました。', 'ERROR')
        return render(request, 'error.html')

###############################################################################
### 関数名： summary_category_view
###############################################################################
@login_required(None, login_url='/P0100Login/')
### def category_view(request, category_code):
def summary_category_view(request, category_code):
    try:
        ...
        #######################################################################
        print_log('[INFO] ########################################', 'INFO')
        print_log('[INFO] P0800Reverse.summary_category_view()関数が開始しました。', 'INFO')
        print_log('[INFO] P0800Reverse.summary_category_view()関数 request = {}'.format(request.method), 'INFO')
        print_log('[INFO] P0800Reverse.summary_category_view()関数 category_code = {}'.format(category_code), 'INFO')
        print_log('[INFO] P0800Reverse.summary_category_view()関数 STEP 1/6.', 'INFO')
        ...
        ### (8)事業所応急対策費 = 事業所数 x 代替活動費
        #######################################################################
        print_log('[INFO] P0800Reverse.summary_category_view()関数 STEP 2/6.', 'INFO')
 
        ####################################################################### 
        ### 延床面積(入力DB)から逆計算により被害建物棟数を求めた結果
        ####################################################################### 
        print_log('[INFO] P0800Reverse.summary_category_view()関数 STEP 4/6.', 'INFO')
        ippan_reverse_list = None
        ippan_reverse_list = IPPAN_VIEW.objects.raw("""
            SELECT 
                IV1.ippan_id AS ippan_id, 
        ...
        ### テンプレートとコンテキストを設定して、レスポンスをブラウザに戻す。
        #######################################################################
        print_log('[INFO] P0800Reverse.summary_category_view()関数 STEP 6/6.', 'INFO')
        ### template = loader.get_template('P0800Reverse/index.html')
        template = loader.get_template('P0800Reverse/summary.html')
        context = {
            'category_code': category_code, 
        ...
        print_log('[INFO] P0800Reverse.summary_category_view()関数が正常終了しました。', 'INFO')
        return HttpResponse(template.render(context, request))
    
    except:
        print_log(sys.exc_info()[0], 'ERROR')
        print_log('[ERROR] P0800Reverse.summary_category_view()関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P0800Reverse.summary_category_view()関数が異常終了しました。', 'ERROR')
        return render(request, 'error.html')
