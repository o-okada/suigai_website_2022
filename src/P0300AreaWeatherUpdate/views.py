...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0300AreaWeatherUpdate/views.py
###############################################################################
...
def index_view(request):
    try:
        ...
        #######################################################################
        print_log('[INFO] ########################################', 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数が開始しました。', 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request = {}'.format(request.method), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request.suigai_id = {}'.format(request.POST.get('suigai_id')), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request.area_id = {}'.format(request.POST.get('area_id')), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request.weather_id = {}'.format(request.POST.get('weather_id')), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request.ken_code = {}'.format(request.POST.get('ken_code')), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 request.city_code = {}'.format(request.POST.get('city_code')), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 1/7.', 'INFO')
        ...
        ### ※関数の内部のネスト数を浅くするため。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 2/7.', 'INFO')
        if request.method == 'GET':
            ken_list = KEN.objects.raw("""SELECT * FROM KEN ORDER BY CAST(KEN_CODE AS INTEGER)""", [])
            template = loader.get_template('P0300AreaWeatherUpdate/index.html')
            context = {
                'ken_list': ken_list, 
            }
            print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数が正常終了しました。', 'INFO')
        ...
        ### ※関数の内部のネスト数を浅くするため。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 3/7.', 'INFO')
        if request.POST.get('suigai_id') is None:
            print_log('[ERROR] P0300AreaWeatherUpdate.index_view()関数でエラーが発生しました。', 'ERROR')
            return render(request, 'error.html')

        if request.POST.get('suigai_id') is None:
            suigai_id = 0
        else:
            suigai_id = request.POST.get('suigai_id')
        
        if request.POST.get('area_id') is None:
            area_id = 0
        else:
            area_id = request.POST.get('area_id')
            
        if request.POST.get('weather_id') is None:
            weather_id = 0
        else:
            weather_id = request.POST.get('weather_id')
            
        if request.POST.get('ken_code') is None:
            ken_code = '0'
        else:
            ken_code = request.POST.get('ken_code')
            
        if request.POST.get('city_code') is None:
            city_code = '0'
        else:
        ...
        ### DBにアクセスして、データを登録する。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 4/7.', 'INFO')
        connection_cursor = connection.cursor()
        try:
            if request.POST.get('area_id') is None and request.POST.get('weather_id') is None:
                print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 4_1/7.', 'INFO')
            ...
            elif request.POST.get('area_id') is None and request.POST.get('weather_id') is not None:
                print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 4_2/7.', 'INFO')
                connection_cursor.execute("""
                    UPDATE SUIGAI SET 
                        area_id=NULL, 
                        weather_id=%s 
            ...
            elif request.POST.get('area_id') is not None and request.POST.get('weather_id') is None:
                print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 4_3/7.', 'INFO')
                connection_cursor.execute("""
                    UPDATE SUIGAI SET 
                        area_id=%s, 
                        weather_id=NULL 
            ...
            elif request.POST.get('area_id') is not None and request.POST.get('weather_id') is not None:
                print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 4_4/7.', 'INFO')
                connection_cursor.execute("""
                    UPDATE SUIGAI SET 
                        area_id=%s, 
                        weather_id=%s 
            ...
            print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 5/7.', 'INFO')
            transaction.commit()
        ...
        ### DBにアクセスして、データを取得する。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 6/7.', 'INFO')
        ken_list = KEN.objects.raw("""SELECT * FROM KEN ORDER BY CAST(KEN_CODE AS INTEGER)""", [])
        if ken_code == '0':
            city_list = []
        else:
        ...
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.index_view()関数 STEP 7/7.', 'INFO')
        template = loader.get_template('P0300AreaWeatherUpdate/index.html')
        context = {
            'ken_code': ken_code, 
            'city_code': city_code, 
        ...
        print_log('[ERROR] P0300AreaWeatherUpdate.index_view()関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P0300AreaWeatherUpdate.index_view()関数が異常終了しました。', 'ERROR')
        return render(request, 'error.html')
...
###############################################################################
@login_required(None, login_url='/P0100Login/')
def ken_city_view(request, ken_code, city_code):
    try:
        #######################################################################
        ### 引数チェック処理(0000)
        ### ブラウザからのリクエストと引数をチェックする。
        #######################################################################
        print_log('[INFO] ########################################', 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数が開始しました。', 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 request = {}'.format(request.method), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 ken_code = {}'.format(ken_code), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 city_code = {}'.format(city_code), 'INFO')
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 STEP 1/3.', 'INFO')
        
        #######################################################################
        ### DBアクセス処理(0010)
        ### DBにアクセスして、データを取得する。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 STEP 2/3.', 'INFO')
        ...
        ### テンプレートとコンテキストを設定して、レスポンスをブラウザに戻す。
        #######################################################################
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数 STEP 3/3.', 'INFO')
        template = loader.get_template('P0300AreaWeatherUpdate/index.html')
        context = {
            'ken_code': ken_code, 
            'city_code': city_code, 
        ...
        print_log('[INFO] P0300AreaWeatherUpdate.ken_city_view()関数が正常終了しました。', 'INFO')
        return HttpResponse(template.render(context, request))
    
    except:
        print_log(sys.exc_info()[0], 'ERROR')
        print_log('[ERROR] P0300AreaWeatherUpdate.ken_city_view()関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P0300AreaWeatherUpdate.ken_city_view()関数が異常終了しました。', 'ERROR')
        return render(request, 'error.html')
