...
### コメントをセルにセットする。
###############################################################################
def add_comment(ws_koeki, ws_result, row, column, fill, com_id):
    ws_koeki.cell(row=row, column=column).fill = fill
    ws_result.cell(row=row, column=column).fill = fill
    
    msg = MESSAGE[com_id][3] + MESSAGE[com_id][4]

    if ws_koeki.cell(row=row, column=column).comment is None:
        ws_koeki.cell(row=row, column=column).comment = Comment(msg, '')
    else:
        ws_koeki.cell(row=row, column=column).comment = Comment(str(ws_koeki.cell(row=row, column=column).comment.text) + msg, '')
...
### template：P0300KoekiUpload/index.html
### (1)GETの場合、公益事業等調査票アップロード画面を表示する。
### (2)POSTの場合、アップロードされた公益事業等調査票ファイルをチェックして、正常ケースの場合、DBに登録する。
### (3)POSTの場合、アップロードされた公益事業等調査票ファイルをチェックして、警告ケースの場合、DBに登録する。
### ※複数EXCELシート未vvvv対応版
###############################################################################
...
def index_view(request):
    try:
        ...
        if request.method == 'GET':
            ...
            return render(request, 'P0300KoekiUpload/index.html', {'form': form})
        ...
        if max_row[0] >= 9:
            for j in range(9, max_row[0] + 1):
                ### セル[9:2]: 水害発生月に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=2).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=2, fill=fill, com_id=50)
                    require_NG.append([ws_koeki[0].title, j, 2, 50])
                else:
                    require_OK.append([ws_koeki[0].title, j, 2, 50])
                    
                ### セル[9:3]: 水害発生日に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=3).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=3, fill=fill, com_id=51)
                    require_NG.append([ws_koeki[0].title, j, 3, 51])
                else:
                    require_OK.append([ws_koeki[0].title, j, 3, 51])
                    
                ### セル[9:4]: 水害発生月に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=4).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=4, fill=fill, com_id=52)
                    require_NG.append([ws_koeki[0].title, j, 4, 52])
                else:
                    require_OK.append([ws_koeki[0].title, j, 4, 52])
                    
                ### セル[9:5]: 水害発生日に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=5).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=5, fill=fill, com_id=53)
                    require_NG.append([ws_koeki[0].title, j, 5, 53])
                else:
                    require_OK.append([ws_koeki[0].title, j, 5, 53])
                    
                ### セル[9:6]: 被害箇所_都道府県名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=6).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=54)
                    require_NG.append([ws_koeki[0].title, j, 6, 54])
                else:
                    require_OK.append([ws_koeki[0].title, j, 6, 54])

                ### セル[9:7]: 被害箇所_都道府県コードに値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=7).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=55)
                    require_NG.append([ws_koeki[0].title, j, 7, 55])
                else:
                    require_OK.append([ws_koeki[0].title, j, 7, 55])

                ### セル[9:8]: 被害箇所_市区町村名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=8).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=56)
                    require_NG.append([ws_koeki[0].title, j, 8, 56])
                else:
                    require_OK.append([ws_koeki[0].title, j, 8, 56])

                ### セル[9:9]: 被害箇所_市区町村コードに値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=9).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=57)
                    require_NG.append([ws_koeki[0].title, j, 9, 57])
                else:
                    require_OK.append([ws_koeki[0].title, j, 9, 57])

                ### セル[9:10]: 被害箇所_町丁名・大字名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=10).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=58)
                    require_NG.append([ws_koeki[0].title, j, 10, 58])
                else:
                    require_OK.append([ws_koeki[0].title, j, 10, 58])

                ### セル[9:11]: 河川・海岸名・地区名_水系・沿岸名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=11).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=59)
                    require_NG.append([ws_koeki[0].title, j, 11, 59])
                else:
                    require_OK.append([ws_koeki[0].title, j, 11, 59])

                ### セル[9:12]: 河川・海岸名・地区名_水系種別に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=12).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=12, fill=fill, com_id=60)
                    require_NG.append([ws_koeki[0].title, j, 12, 60])
                else:
                    require_OK.append([ws_koeki[0].title, j, 12, 60])

                ### セル[9:13]: 河川・海岸名・地区名_河川・海岸名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=13).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=61)
                    require_NG.append([ws_koeki[0].title, j, 13, 61])
                else:
                    require_OK.append([ws_koeki[0].title, j, 13, 61])

                ### セル[9:14]: 河川・海岸名・地区名_河川種別に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=14).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=62)
                    require_NG.append([ws_koeki[0].title, j, 14, 62])
                else:
                    require_OK.append([ws_koeki[0].title, j, 14, 62])

                ### セル[9:15]: 工種区分に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=15).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=63)
                    require_NG.append([ws_koeki[0].title, j, 15, 63])
                else:
                    require_OK.append([ws_koeki[0].title, j, 15, 63])

                ### セル[9:16]: 水害原因コード1に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=16).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=64)
                    require_NG.append([ws_koeki[0].title, j, 16, 64])
                else:
                    require_OK.append([ws_koeki[0].title, j, 16, 64])

                ### セル[9:17]: 水害原因コード2に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=17).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=65)
                    require_NG.append([ws_koeki[0].title, j, 17, 65])
                else:
                    require_OK.append([ws_koeki[0].title, j, 17, 65])

                ### セル[9:18]: 水害原因コード3に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=18).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=18, fill=fill, com_id=66)
                    require_NG.append([ws_koeki[0].title, j, 18, 66])
                else:
                    require_OK.append([ws_koeki[0].title, j, 18, 66])

                ### セル[9:19]: 異常気象コードに値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=19).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=19, fill=fill, com_id=67)
                    require_NG.append([ws_koeki[0].title, j, 19, 67])
                else:
                    require_OK.append([ws_koeki[0].title, j, 19, 67])

                ### セル[9:20]: 事業コードに値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=20).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=20, fill=fill, com_id=68)
                    require_NG.append([ws_koeki[0].title, j, 20, 68])
                else:
                    require_OK.append([ws_koeki[0].title, j, 20, 68])

                ### セル[9:21]: 調査対象機関名称に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=21).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=21, fill=fill, com_id=69)
                    require_NG.append([ws_koeki[0].title, j, 21, 69])
                else:
                    require_OK.append([ws_koeki[0].title, j, 21, 69])

                ### セル[9:22]: 被害金額_物的被害額(千円)に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=22).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=22, fill=fill, com_id=70)
                    require_NG.append([ws_koeki[0].title, j, 22, 70])
                else:
                    require_OK.append([ws_koeki[0].title, j, 22, 70])

                ### セル[9:23]: 被害金額_営業停止に伴う売上減少額に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=23).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=23, fill=fill, com_id=71)
                    require_NG.append([ws_koeki[0].title, j, 23, 71])
                else:
                    require_OK.append([ws_koeki[0].title, j, 23, 71])

                ### セル[9:24]: 被害金額_代替活動費(外注費)に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=24).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=24, fill=fill, com_id=72)
                    require_NG.append([ws_koeki[0].title, j, 24, 72])
                else:
                    require_OK.append([ws_koeki[0].title, j, 24, 72])

                ### セル[9:25]: 被害金額_その他に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=25).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=25, fill=fill, com_id=73)
                    require_NG.append([ws_koeki[0].title, j, 25, 73])
                else:
                    require_OK.append([ws_koeki[0].title, j, 25, 73])

                ### セル[9:26]: 被害金額_営業停止損失額合計(千円)に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=26).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=26, fill=fill, com_id=74)
                    require_NG.append([ws_koeki[0].title, j, 26, 74])
                else:
                    require_OK.append([ws_koeki[0].title, j, 26, 74])

                ### セル[9:27]: 営業停止期間等_営業停止期間日に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=27).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=27, fill=fill, com_id=75)
                    require_NG.append([ws_koeki[0].title, j, 27, 75])
                else:
                    require_OK.append([ws_koeki[0].title, j, 27, 75])

                ### セル[9:28]: 営業停止期間等_営業停止期間時間に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=28).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=28, fill=fill, com_id=76)
                    require_NG.append([ws_koeki[0].title, j, 28, 76])
                else:
                    require_OK.append([ws_koeki[0].title, j, 28, 76])

                ### セル[9:29]: 営業停止期間等_停止数量に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=29).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=29, fill=fill, com_id=77)
                    require_NG.append([ws_koeki[0].title, j, 29, 77])
                else:
                    require_OK.append([ws_koeki[0].title, j, 29, 77])

                ### セル[9:30]: 照会先_調査担当課名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=30).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=30, fill=fill, com_id=78)
                    require_NG.append([ws_koeki[0].title, j, 30, 78])
                else:
                    require_OK.append([ws_koeki[0].title, j, 30, 78])

                ### セル[9:31]: 照会先_調査担当者名に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=31).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=31, fill=fill, com_id=79)
                    require_NG.append([ws_koeki[0].title, j, 31, 79])
                else:
                    require_OK.append([ws_koeki[0].title, j, 31, 79])

                ### セル[9:32]: 照会先_電話番号に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=32).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=32, fill=fill, com_id=80)
                    require_NG.append([ws_koeki[0].title, j, 32, 80])
                else:
                    require_OK.append([ws_koeki[0].title, j, 32, 80])

                ### セル[9:33]: 備考に値がセットされていることをチェックする。
                if ws_koeki[0].cell(row=j, column=33).value is None:
                    add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=33, fill=fill, com_id=81)
                    require_NG.append([ws_koeki[0].title, j, 33, 81])
                else:
                    require_OK.append([ws_koeki[0].title, j, 33, 81])
        ...
        if max_row[0] >= 9:
            for j in range(9, max_row[0] + 1):
                ### セル[9:2]: 水害発生月について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=2).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=2).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=2, fill=fill, com_id=150)
                        format_NG.append([ws_koeki[0].title, j, 2, 150])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 2, 150])
                    
                ### セル[9:3]: 水害発生日について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=3).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=3).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=3, fill=fill, com_id=151)
                        format_NG.append([ws_koeki[0].title, j, 3, 151])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 3, 151])
                    
                ### セル[9:4]: 水害発生月について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=4).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=4).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=4, fill=fill, com_id=152)
                        format_NG.append([ws_koeki[0].title, j, 4, 152])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 4, 152])
                    
                ### セル[9:5]: 水害発生日について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=5).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=5).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=5, fill=fill, com_id=153)
                        format_NG.append([ws_koeki[0].title, j, 5, 153])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 5, 153])
                    
                ### セル[9:6]: 被害箇所_都道府県名について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=6).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=6).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=6).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=154)
                        format_NG.append([ws_koeki[0].title, j, 6, 154])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 6, 154])
                    
                ### セル[9:7]: 被害箇所_都道府県コードについて形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=7).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=7).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=7).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=155)
                        format_NG.append([ws_koeki[0].title, j, 7, 155])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 7, 155])
                    
                ### セル[9:8]: 被害箇所_市区町村名について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=8).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=8).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=8).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=156)
                        format_NG.append([ws_koeki[0].title, j, 8, 156])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 8, 156])
                    
                ### セル[9:9]: 被害箇所_市区町村コードについて形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=9).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=9).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=9).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=157)
                        format_NG.append([ws_koeki[0].title, j, 9, 157])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 9, 157])
                    
                ### セル[9:10]: 被害箇所_町丁名・大字名について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=10).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=10).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=10).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=158)
                        format_NG.append([ws_koeki[0].title, j, 10, 158])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 10, 158])
                    
                ### セル[9:11]: 河川・海岸名・地区名_水系・沿岸名について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=11).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=11).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=11).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=159)
                        format_NG.append([ws_koeki[0].title, j, 11, 159])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 11, 159])
                    
                ### セル[9:12]: 河川・海岸名・地区名_水系種別について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=12).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=12).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=12).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=12, fill=fill, com_id=160)
                        format_NG.append([ws_koeki[0].title, j, 12, 160])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 12, 160])
                    
                ### セル[9:13]: 河川・海岸名・地区名_河川・海岸名について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=13).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=13).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=13).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=161)
                        format_NG.append([ws_koeki[0].title, j, 13, 161])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 13, 161])
                    
                ### セル[9:14]: 河川・海岸名・地区名_河川種別について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=14).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=14).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=14).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=162)
                        format_NG.append([ws_koeki[0].title, j, 14, 162])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 14, 162])
                    
                ### セル[9:15]: 工種区分について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=15).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=15).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=15).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=163)
                        format_NG.append([ws_koeki[0].title, j, 15, 163])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 15, 163])
                    
                ### セル[9:16]: 水害原因コード1について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=16).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=16).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=16).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=164)
                        format_NG.append([ws_koeki[0].title, j, 16, 164])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 16, 164])
                    
                ### セル[9:17]: 水害原因コード2について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=17).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=17).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=17).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=165)
                        format_NG.append([ws_koeki[0].title, j, 17, 165])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 17, 165])
                    
                ### セル[9:18]: 水害原因コード3について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=18).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=18).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=18).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=18, fill=fill, com_id=166)
                        format_NG.append([ws_koeki[0].title, j, 18, 166])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 18, 166])
                    
                ### セル[9:19]: 異常気象コードについて形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=19).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=19).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=19).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=19, fill=fill, com_id=167)
                        format_NG.append([ws_koeki[0].title, j, 19, 167])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 19, 167])
                    
                ### セル[9:20]: 事業コードについて形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=20).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=20).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=20).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=20, fill=fill, com_id=168)
                        format_NG.append([ws_koeki[0].title, j, 20, 168])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 20, 168])
                    
                ### セル[9:21]: 調査対象機関名称について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=21).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=21).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=21).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=21, fill=fill, com_id=169)
                        format_NG.append([ws_koeki[0].title, j, 21, 169])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 21, 169])
                    
                ### セル[9:22]: 被害金額_物的被害額(千円)について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=22).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=22).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=22).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=22, fill=fill, com_id=170)
                        format_NG.append([ws_koeki[0].title, j, 22, 170])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 22, 170])
                    
                ### セル[9:23]: 被害金額_営業停止に伴う売上減少額について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=23).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=23).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=23).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=23, fill=fill, com_id=171)
                        format_NG.append([ws_koeki[0].title, j, 23, 171])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 23, 171])
                    
                ### セル[9:24]: 被害金額_代替活動費(外注費)について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=24).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=24).value, int) == False and \
                        isinstance(ws_koeki[0].cell(row=j, column=24).value, float) == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=24, fill=fill, com_id=172)
                        format_NG.append([ws_koeki[0].title, j, 24, 172])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 24, 172])
                    
                ### セル[9:25]: 被害金額_その他について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=25).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=25).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=25, fill=fill, com_id=173)
                        format_NG.append([ws_koeki[0].title, j, 25, 173])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 25, 173])
                    
                ### セル[9:26]: 被害金額_営業停止損失額合計(千円)について形式が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=26).value is None:
                    pass
                else:
                    if split_name_code(ws_koeki[0].cell(row=j, column=26).value)[-1].isdecimal() == False:
                        add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=26, fill=fill, com_id=174)
                        format_NG.append([ws_koeki[0].title, j, 26, 174])
                    else:
                        format_OK.append([ws_koeki[0].title, j, 26, 174])
                ...
                ### セル[9:30]: 照会先_調査担当課名について形式が正しいことをチェックする。
                ### セル[9:31]: 照会先_調査担当者名について形式が正しいことをチェックする。
        ...
        if max_row[0] >= 9:
            for j in range(9, max_row[0] + 1):
                ### セル[9:2]: 水害発生月について範囲が正しいことをチェックする。
                ### セル[9:3]: 水害発生日について範囲が正しいことをチェックする。
                ### セル[9:4]: 水害発生月について範囲が正しいことをチェックする。
                ### セル[9:5]: 水害発生日について範囲が正しいことをチェックする。

                ### セル[9:6]: 被害箇所_都道府県名について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=6).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=6).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=6).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=6).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=254)
                            range_NG.append([ws_koeki[0].title, j, 6, 254])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 6, 254])

                ### セル[9:7]: 被害箇所_都道府県コードについて範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=7).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=7).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=7).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=7).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=255)
                            range_NG.append([ws_koeki[0].title, j, 7, 255])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 7, 255])
                
                ### セル[9:8]: 被害箇所_市区町村名について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=8).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=8).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=8).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=8).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=256)
                            range_NG.append([ws_koeki[0].title, j, 8, 256])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 8, 256])

                ### セル[9:9]: 被害箇所_市区町村コードについて範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=9).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=9).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=9).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=9).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=257)
                            range_NG.append([ws_koeki[0].title, j, 9, 257])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 9, 257])

                ### セル[9:10]: 被害箇所_町丁名・大字名について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=10).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=10).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=10).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=10).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=258)
                            range_NG.append([ws_koeki[0].title, j, 10, 258])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 10, 258])

                ### セル[9:11]: 河川・海岸名・地区名_水系・沿岸名について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=11).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=11).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=11).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=11).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=259)
                            range_NG.append([ws_koeki[0].title, j, 11, 259])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 11, 259])

                ### セル[9:12]: 河川・海岸名・地区名_水系種別について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=12).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=12).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=12).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=12).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=12, fill=fill, com_id=260)
                            range_NG.append([ws_koeki[0].title, j, 12, 260])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 12, 260])

                ### セル[9:13]: 河川・海岸名・地区名_河川・海岸名について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=13).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=13).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=13).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=13).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=261)
                            range_NG.append([ws_koeki[0].title, j, 13, 261])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 13, 261])
                
                ### セル[9:14]: 河川・海岸名・地区名_河川種別について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=14).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=14).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=14).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=14).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=262)
                            range_NG.append([ws_koeki[0].title, j, 14, 262])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 14, 262])

                ### セル[9:15]: 工種区分について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=15).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=15).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=15).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=15).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=263)
                            range_NG.append([ws_koeki[0].title, j, 15, 263])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 15, 263])

                ### セル[9:16]: 水害原因コード1について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=16).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=16).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=16).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=16).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=264)
                            range_NG.append([ws_koeki[0].title, j, 16, 264])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 16, 264])

                ### セル[9:17]: 水害原因コード2について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=17).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=17).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=17).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=17).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=265)
                            range_NG.append([ws_koeki[0].title, j, 17, 265])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 17, 265])

                ### セル[9:18]: 水害原因コード3について範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=18).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=18).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=18).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=18).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=18, fill=fill, com_id=266)
                            range_NG.append([ws_koeki[0].title, j, 18, 266])
                        else:
                            range_OK.append([ws_koeki[0].title, j, 18, 266])

                ### セル[9:19]: 異常気象コードについて範囲が正しいことをチェックする。
                if ws_koeki[0].cell(row=j, column=19).value is None:
                    pass
                else:
                    if isinstance(ws_koeki[0].cell(row=j, column=19).value, int) == True or \
                        isinstance(ws_koeki[0].cell(row=j, column=19).value, float) == True:
                        if float(ws_koeki[0].cell(row=j, column=19).value) < 0:
                            add_comment(ws_koeki=ws_koeki[0], ws_result=ws_result[0], row=j, column=19, fill=fill, com_id=267)
                            range_NG.append([ws_koeki[0].title, j, 19, 267])
                        else:
                ...
                ### セル[9:30]: 照会先_調査担当課名について範囲が正しいことをチェックする。
                ### セル[9:31]: 照会先_調査担当者名について範囲が正しいことをチェックする。
        ...
        if len(require_NG_list) > 0 or \
            ...
            try:
                ...
                connection_cursor.execute("""
                    ...
                    """, [
                        split_name_code(ws_koeki[0].cell(row=7, column=3).value)[-1], 
                    ])
                ...
                connection_cursor.execute("""
                    INSERT INTO TRIGGER (
                        ...
                        convert_empty_to_none(split_name_code(ws_koeki[0].cell(row=7, column=2).value)[-1]), ### ken_code 
                        convert_empty_to_none(split_name_code(ws_koeki[0].cell(row=7, column=3).value)[-1]), ### city_code 
        ...
        try:
            ...
            for i, _ in enumerate(ws_koeki):
                ...
                print_log('[DEBUG] P0300KoekiUpload.index_view()関数 suigai_id = {}'.format(suigai_id), 'DEBUG')
                ...
                connection_cursor.execute("""
                    INSERT INTO SUIGAI (
                        ...
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=2).value)[-1]), ### ken_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=3).value)[-1]), ### city_code 
                        convert_empty_to_none(ws_koeki[i].cell(row=7, column=4).value), ### begin_date 
                        convert_empty_to_none(ws_koeki[i].cell(row=7, column=5).value), ### end_date 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=6).value)[-1]), ### cause_1_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=7).value)[-1]), ### cause_2_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=8).value)[-1]), ### cause_3_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=7, column=9).value)[-1]), ### area_id 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=10, column=2).value)[-1]), ### suikei_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=10, column=4).value)[-1]), ### kasen_code 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=10, column=6).value)[-1]), ### gradient_code 
                        convert_empty_to_none(ws_koeki[i].cell(row=14, column=2).value), ### residential_area 
                        convert_empty_to_none(ws_koeki[i].cell(row=14, column=3).value), ### agricultural_area 
                        convert_empty_to_none(ws_koeki[i].cell(row=14, column=4).value), ### underground_area 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=14, column=6).value)[-1]), ### kasen_kaigan_code 
                        convert_empty_to_none(ws_koeki[i].cell(row=14, column=8).value), ### crop_damaga 
                        convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=14, column=10).value)[-1]), ### weather_id 
                        None, ### deleted_at 
                        upload_file_path, ### upload_file_path 
                        upload_file_name, ### upload_file_name 
                    ...
                    for j in range(20, max_row[i] + 1):
                        ...
                        connection_cursor.execute(""" 
                            INSERT INTO IPPAN (
                                ...
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=2).value), ### ippan_name
                                suigai_id, ### suigai_id
                                convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=j, column=3).value)[-1]), ### building_code
                                convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=j, column=4).value)[-1]), ### underground_code
                                convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=j, column=5).value)[-1]), ### flood_sediment_code
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=6).value), ### building_lv00
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=7).value), ### building_lv01_49
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=8).value), ### building_lv50_99
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=9).value), ### building_lv100
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=10).value), ### building_half
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=11).value), ### building_full
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=12).value), ### floor_area
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=13).value), ### family
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=14).value), ### office
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=15).value), ### farmer_fisher_lv00
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=16).value), ### farmer_fisher_lv01_49
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=17).value), ### farmer_fisher_lv50_99
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=18).value), ### farmer_fisher_lv100
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=19).value), ### farmer_fisher_full
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=20).value), ### employee_lv00
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=21).value), ### employee_lv01_49
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=22).value), ### employee_lv50_99
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=23).value), ### employee_lv100
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=24).value), ### employee_full
                                convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=j, column=25).value)[-1]), ### industry_code
                                convert_empty_to_none(split_name_code(ws_koeki[i].cell(row=j, column=26).value)[-1]), ### usage_code
                                convert_empty_to_none(ws_koeki[i].cell(row=j, column=27).value), ### comment
                                None, ### deleted_at 
                            ])
