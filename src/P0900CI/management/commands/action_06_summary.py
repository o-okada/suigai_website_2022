...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 1/.', 'INFO')
    
            ###################################################################
            ### DBアクセス処理(0010)
            ###################################################################
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 2/.', 'INFO')
            trigger_list = None
            suigai_id_list = None
            ...
            ### 
            ###################################################################
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 3/.', 'INFO')
            trigger_list = TRIGGER.objects.raw("""SELECT * FROM TRIGGER WHERE ACTION_CODE='6' AND CONSUMED_AT IS NULL ORDER BY CAST(TRIGGER_ID AS INTEGER) LIMIT 1""", [])
            ...
            ### 
            ###################################################################
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 4/.', 'INFO')
            if suigai_id_list:
                if len(suigai_id_list) > 0:
                    print_log('suigai_id_list[0] = {}'.format(suigai_id_list[0]), 'INFO')
                    connection_cursor.execute("""
                        DELETE FROM IPPAN_SUMMARY WHERE suigai_id=%s""", [
                        suigai_id_list[0],])
            
            ###################################################################
            ### 
            ###################################################################
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 5/.', 'INFO')
            if suigai_id_list:
                if len(suigai_id_list) > 0:
                    print_log('suigai_id_list[0] = {}'.format(suigai_id_list[0]), 'INFO')
                    connection_cursor.execute("""
                    ...
                    ########################################################### 
                    print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 6/.', 'INFO')
                    connection_cursor.execute("""
                        UPDATE TRIGGER SET 
                    ...
                    ########################################################### 
                    print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 7/.', 'INFO')
                    connection_cursor.execute("""
                    ...
                    ### 失敗の場合は、次のトリガーを発行しない。
                    ########################################################### 
                    print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 8/.', 'INFO')
                    connection_cursor.execute("""
            ...
            ### 戻り値セット処理(0020)
            ###################################################################
            print_log('[INFO] P0900CI.action_06_summary.handle()関数 STEP 9/.', 'INFO')
