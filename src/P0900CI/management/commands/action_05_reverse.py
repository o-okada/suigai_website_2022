...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 1/10.', 'INFO')
            ...
            ###################################################################
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 2/10.', 'INFO')
            trigger_list = None
            ...
            trigger_list = TRIGGER.objects.raw("""
                SELECT 
                    * 
                FROM TRIGGER 
                WHERE ACTION_CODE='5' AND CONSUMED_AT IS NULL 
            ...
            ### ※ネストを浅くするために、処理対象外の場合、処理を終了させる。
            ###################################################################
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 3/10.', 'INFO')
            ...
            ### ※ネストを浅くするために、処理対象外の場合、処理を終了させる。
            ###################################################################
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 4/10.', 'INFO')
            ...
            if ippan_reverse_list is None:
                print_log('[INFO] P0900CI.action_05_reverse.handle()関数が警告終了しました。', 'INFO')
                return 4
            ...
            ###################################################################
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 5/10.', 'INFO')
            success_count = 0
            failure_count = 0
            epsilon = 0.0000001
            ...
            ################################################################### 
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 6/10.', 'INFO')
            ...
            ################################################################### 
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 7/10.', 'INFO')
            ...
            ################################################################### 
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 8/10.', 'INFO')
            print_log('suigai_id_list[0] = {}'.format(suigai_id_list[0]), 'INFO')
            print_log('repository_id_list[0] = {}'.format(repository_id_list[0]), 'INFO')
            if failure_count == 0:
                connection_cursor.execute("""
            ...
            ### DBアクセス処理(0110)
            ################################################################### 
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 9/10.', 'INFO')
            transaction.commit()

            ###################################################################
            ### 戻り値セット処理(0120)
            ###################################################################
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数 STEP 10/10.', 'INFO')
            print_log('[INFO] P0900CI.action_05_reverse.handle()関数が正常終了しました。', 'INFO')
            return 0
