...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_Q01_download_chitan.py
### Q01：ダウンロード
###############################################################################
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            ###################################################################
            reset_log()
            print_log('[INFO] P0900Action.action_Q01_download_chitan.handle()関数が開始しました。', 'INFO')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 1/13.', 'DEBUG')
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 2/13.', 'DEBUG')
            ...
            if trigger_list is None:
                print_log('[INFO] P0900Action.action_Q01_download_chitan.handle()関数が正常終了しました。', 'INFO')
                return 0
            ...
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].trigger_id = {}'.format(trigger_list[0].trigger_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].suigai_id = {}'.format(trigger_list[0].suigai_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].city_code = {}'.format(trigger_list[0].city_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].ken_code = {}'.format(trigger_list[0].ken_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].download_file_path = {}'.format(trigger_list[0].download_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].download_file_name = {}'.format(trigger_list[0].download_file_name), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].upload_file_path = {}'.format(trigger_list[0].upload_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 trigger_list[0].upload_file_name = {}'.format(trigger_list[0].upload_file_name), 'DEBUG')
    
            ###################################################################
            ### DBアクセス処理(0020)
            ### マスタデータを取得する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 3/13.', 'DEBUG')
            ### 1010: 都道府県シート
            ...
            ### ダウンロード用ファイルパスをセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 4/13.', 'DEBUG')
            ...
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 download_file_path = {}'.format(download_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 download_file_name = {}'.format(download_file_name), 'DEBUG')

            ###################################################################
            ### EXCEL入出力処理(0040)
            ### ワークシートを局所変数にセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 5/13.', 'DEBUG')
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 6/13.', 'DEBUG')
            ...
            ### マスタ用のシートに値をセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 7/13.', 'DEBUG')
            ### 1010: 都道府県シート
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 8/13.', 'DEBUG')
            ### 1020: 市区町村VLOOKUP
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 9/13.', 'DEBUG')
            ...
            ### ws_ippan.cell(row=6, column=6).value = '代表被災地区名[全角]'
            ### ws_ippan.cell(row=6, column=7).value = '都道府県名[全角]'
            ### ws_ippan.cell(row=6, column=8).value = '市区町村名[全角]'
            ### ws_ippan.cell(row=6, column=9).value = '都道府県コード'
            ### ws_ippan.cell(row=6, column=10).value = ''
            ### ws_ippan.cell(row=6, column=11).value = '異常気象コード'
            ### ws_ippan.cell(row=6, column=12).value = '水害発生'
            ### ws_ippan.cell(row=6, column=13).value = ''
            ### ws_ippan.cell(row=6, column=14).value = ''
            ### ws_ippan.cell(row=6, column=15).value = ''
            ### ws_ippan.cell(row=6, column=16).value = '工種区分'
            ### ws_ippan.cell(row=6, column=17).value = ''
            ### ws_ippan.cell(row=6, column=18).value = '市区町村コード'
            ### ws_ippan.cell(row=6, column=19).value = '災害復旧'
            ### ws_ippan.cell(row=6, column=20).value = ''
            ### ws_ippan.cell(row=6, column=21).value = '備考'
            ...
            ### ws_ippan.cell(row=7, column=7).value = ''
            ### ws_ippan.cell(row=7, column=8).value = ''
            ### ws_ippan.cell(row=7, column=9).value = ''
            ### ws_ippan.cell(row=7, column=10).value = ''
            ### ws_ippan.cell(row=7, column=11).value = ''
            ### ws_ippan.cell(row=7, column=12).value = '月'
            ### ws_ippan.cell(row=7, column=13).value = '日'
            ...
            ### ws_ippan.cell(row=7, column=19).value = '箇所'
            ### ws_ippan.cell(row=7, column=20).value = '査定額(千円)'
            ...
            ### EXCELのセルに、値を埋め込む。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 10/13.', 'DEBUG')
            chitan_list = CHITAN.objects.raw("""
                SELECT 
                    CH1.chitan_id AS chitan_id,
                    CH1.chitan_name AS chitan_name,
                    CH1.chitan_file_id AS chitan_file_id, 
                    ...
                    CH1.suikei_code AS suikei_code, 
                    SK1.suikei_name AS suikei_name, 
                    CH1.kasen_code AS kasen_code, 
                    ...
                    CH1.kasen_kaigan_code AS kasen_kaigan_code,
                ...
                WHERE 
                    CH1.ken_code=%s AND CH1.deleted_at IS NULL 
                ORDER BY CAST(CH1.city_code AS INTEGER), CAST(CH1.chitan_file_id AS INTEGER), CAST(CH1.chitan_id AS INTEGER)""", [
                    trigger_list[0].ken_code, 
                ])
    
            if chitan_list:
                for i, chitan in enumerate(chitan_list):
                    if chitan.chitan_id is not None and len(str(chitan.chitan_id)) > 0:
                        ws_chitan.cell(row=8+i, column=1).value = str(chitan.chitan_id)
                        ws_chitan.cell(row=8+i, column=1).fill = green_fill
                    
                    if chitan.suikei_name is not None and chitan.suikei_code is not None and len(str(chitan.suikei_name)) > 0 and len(str(chitan.suikei_code)) > 0:
                        ws_chitan.cell(row=8+i, column=2).value = str(chitan.suikei_name) + ":" + str(chitan.suikei_code)

                    if chitan.suikei_type_name is not None and chitan.suikei_type_code is not None and len(str(chitan.suikei_type_name)) > 0 and len(str(chitan.suikei_type_code)) > 0:                        
                        ws_chitan.cell(row=8+i, column=3).value = str(chitan.suikei_type_name) + ":" + str(chitan.suikei_type_code)
                    
                    if chitan.kasen_name is not None and chitan.kasen_code is not None and len(str(chitan.kasen_name)) > 0 and len(str(chitan.kasen_code)) > 0:
                        ws_chitan.cell(row=8+i, column=4).value = str(chitan.kasen_name) + str(chitan.kasen_code)
                        
                    if chitan.kasen_type_name is not None and chitan.kasen_type_code is not None and len(str(chitan.kasen_type_name)) > 0 and len(str(chitan.kasen_type_code)) > 0:
                        ws_chitan.cell(row=8+i, column=5).value = str(chitan.kasen_type_name) + ":" + str(chitan.kasen_type_code)
                    
                    if chitan.chitan_name is not None and len(str(chitan.chitan_name)) > 0:
                        ws_chitan.cell(row=8+i, column=6).value = str(chitan.chitan_name)
                        
                    if chitan.ken_name is not None and chitan.ken_code is not None and len(str(chitan.ken_name)) > 0 and len(str(chitan.ken_code)) > 0:
                        ws_chitan.cell(row=8+i, column=7).value = str(chitan.ken_name) + ":" + str(chitan.ken_code)
                    
                    if chitan.city_name is not None and chitan.city_code is not None and len(str(chitan.city_name)) > 0 and len(str(chitan.city_code)) > 0:
                        ws_chitan.cell(row=8+i, column=8).value = str(chitan.city_name) + ":" + str(chitan.city_code)
                    
                    if chitan.ken_code is not None and len(str(chitan.ken_code)) > 0:
                        ws_chitan.cell(row=8+i, column=9).value = str(chitan.ken_code)

                    ws_chitan.cell(row=8+i, column=10).value = ""
                    
                    if chitan.weather_name is not None and chitan.weather_id is not None and len(str(chitan.weather_name)) > 0 and len(str(chitan.weather_id)) > 0:
                        ws_chitan.cell(row=8+i, column=11).value = str(chitan.weather_name) + ":" + str(chitan.weather_id)
                        
                    if chitan.begin_month is not None and len(str(chitan.begin_month)) > 0:
                        ws_chitan.cell(row=8+i, column=12).value = str(chitan.begin_month)

                    if chitan.begin_day is not None and len(str(chitan.begin_day)) > 0:
                        ws_chitan.cell(row=8+i, column=13).value = str(chitan.begin_day)
                    
                    if chitan.end_month is not None and len(str(chitan.end_month)) > 0:
                        ws_chitan.cell(row=8+i, column=14).value = str(chitan.end_month)
                    
                    if chitan.end_day is not None and len(str(chitan.end_day)) > 0:
                        ws_chitan.cell(row=8+i, column=15).value = str(chitan.end_day)
                        
                    if chitan.kasen_kaigan_name is not None and chitan.kasen_kaigan_code is not None and len(str(chitan.kasen_kaigan_name)) > 0 and len(str(chitan.kasen_kaigan_code)) > 0:
                        ws_chitan.cell(row=8+i, column=16).value = str(chitan.kasen_kaigan_name) + ":" + str(chitan.kasen_kaigan_code)

                    ws_chitan.cell(row=8+i, column=17).value = ""
                        
                    if chitan.city_code is not None and len(str(chitan.city_code)) > 0:
                        ws_chitan.cell(row=8+i, column=18).value = str(chitan.city_code)
                        
                    if chitan.point is not None and len(str(chitan.point)) > 0:
                        ws_chitan.cell(row=8+i, column=19).value = str(chitan.point)
                        
                    if chitan.assessment is not None and len(str(chitan.assessment)) > 0:
                        ws_chitan.cell(row=8+i, column=20).value = str(chitan.assessment)
                    
                    if chitan.comment is not None and len(str(chitan.comment)) > 0:
                        ws_chitan.cell(row=8+i, column=21).value = str(chitan.comment)

            ###################################################################
            ### EXCEL入出力処理(0100)
            ### ダウンロード用のEXCELファイルを保存する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan()関数 STEP 11/13.', 'DEBUG')
            ...
            ### トリガーデータを更新する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 12/13.', 'DEBUG')
            ...
            try:
                ...
                print_log('[ERROR] P0900Action.action_Q01_download_chitan.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            ...
            ### 戻り値を戻す。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_Q01_download_chitan.handle()関数 STEP 13/13.', 'DEBUG')
            print_log('[INFO] P0900Action.action_Q01_download_chitan.handle()関数が正常終了しました。', 'INFO')
            return 0
        except:
            print_log('[ERROR] P0900Action.action_Q01_download_chitan.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            print_log('[ERROR] P0900Action.action_Q01_download_chitan.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_Q01_download_chitan.handle()関数が異常終了しました。', 'ERROR')
            return 8
