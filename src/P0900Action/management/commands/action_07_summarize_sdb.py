...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_07_summarize_sdb.py
###############################################################################
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            connection_cursor.execute("""
                ...
                WHERE 
                    suigai_id=%s AND 
                    deleted_at IS NULL""", [trigger_list[0].suigai_id, ])
            ...
            connection_cursor.execute("""
                ...
                WHERE 
                    IV1.suigai_id=%s AND 
                    IV1.deleted_at IS NULL""", [trigger_list[0].suigai_id, ])
            ...
            print_log('[ERROR] P0900Action.action_07_summarize_sdb.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_07_summarize_sdb.handle()関数が異常終了しました。', 'ERROR')
            return 8
