...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_D01_download_ippan_ken.py
### D01：ダウンロード
###############################################################################
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            ###################################################################
            reset_log()
            print_log('[INFO] P0900Action.action_D01_download_ippan_ken.handle()関数が開始しました。', 'INFO')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 1/13.', 'DEBUG')
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 2/13.', 'DEBUG')
            ...
            if trigger_list is None:
                print_log('[INFO] P0900Action.action_D01_download_ippan_ken.handle()関数が正常終了しました。', 'INFO')
                return 0
            ...
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].trigger_id = {}'.format(trigger_list[0].trigger_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].suigai_id = {}'.format(trigger_list[0].suigai_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].city_code = {}'.format(trigger_list[0].city_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].ken_code = {}'.format(trigger_list[0].ken_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].download_file_path = {}'.format(trigger_list[0].download_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].download_file_name = {}'.format(trigger_list[0].download_file_name), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].upload_file_path = {}'.format(trigger_list[0].upload_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 trigger_list[0].upload_file_name = {}'.format(trigger_list[0].upload_file_name), 'DEBUG')
    
            ###################################################################
            ### DBアクセス処理(0020)
            ### マスタデータを取得する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 3/13.', 'DEBUG')
            ### 1000: 建物区分シート
            ### 1010: 都道府県シート
            ...
            ### ダウンロード用ファイルパスをセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 4/13.', 'DEBUG')
            ...
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 download_file_path = {}'.format(download_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 download_file_name = {}'.format(download_file_name), 'DEBUG')

            ###################################################################
            ### EXCEL入出力処理(0040)
            ### ワークシートを局所変数にセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 5/13.', 'DEBUG')
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 6/13.', 'DEBUG')
            ...
            ### マスタ用のシートに値をセットする。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 7/13.', 'DEBUG')
            ### 1000: 建物区分シート
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 8/13.', 'DEBUG')
            ### 1020: 市区町村VLOOKUP
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 9/13.', 'DEBUG')
            ...
            ### ws_ippan.cell(row=6, column=48).value = '備考1'
            ### ws_ippan.cell(row=6, column=49).value = '備考2'
            ### ws_ippan.cell(row=6, column=50).value = '水害区域面積(宅地)'
            ### ws_ippan.cell(row=6, column=51).value = '水害区域面積(農地)'
            ### ws_ippan.cell(row=6, column=52).value = '水害区域面積(地下)'
            ### ws_ippan.cell(row=6, column=53).value = '工種'
            ### ws_ippan.cell(row=6, column=54).value = '農作物被害額(千円)'
            ### ws_ippan.cell(row=6, column=55).value = '異常気象コード'
            ### ws_ippan.cell(row=6, column=56).value = '調査票シート名'
            ### ws_ippan.cell(row=6, column=57).value = ''
            ### ws_ippan.cell(row=6, column=58).value = 'ファイル最終更新日時'
            ### ws_ippan.cell(row=6, column=59).value = 'ファイルパス'
            ...
            ### EXCELのセルに、値を埋め込む。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 10/13.', 'DEBUG')
            ippan_view_list = IPPAN_VIEW.objects.raw("""
                ...
                FROM IPPAN_VIEW IV1 
                WHERE 
                    IV1.ken_code=%s AND IV1.deleted_at IS NULL 
                ORDER BY CAST(IV1.city_code AS INTEGER), CAST(IV1.suigai_id AS INTEGER), CAST(IV1.ippan_id AS INTEGER)""", [
            ...
            if ippan_view_list:
                for i, ippan in enumerate(ippan_view_list):
                    ...
                    ws_ippan.cell(row=8+i, column=44).value = ""
                    ws_ippan.cell(row=8+i, column=45).value = ""
                    ws_ippan.cell(row=8+i, column=46).value = ""
                    ws_ippan.cell(row=8+i, column=47).value = ""
                    
                    if ippan.comment is not None and len(str(ippan.comment)) > 0:
                        ws_ippan.cell(row=8+i, column=48).value = str(ippan.comment)
                    
                    ### TO-DO TODO TO_DO to-do todo to_do
                    if ippan.comment is not None and len(str(ippan.comment)) > 0:
                        ws_ippan.cell(row=8+i, column=49).value = str(ippan.comment)
                    
                    if ippan.residential_area is not None and len(str(ippan.residential_area)) > 0:
                        ws_ippan.cell(row=8+i, column=50).value = str(ippan.residential_area)
                    
                    if ippan.agricultural_area is not None and len(str(ippan.agricultural_area)) > 0:
                        ws_ippan.cell(row=8+i, column=51).value = str(ippan.agricultural_area)
                    
                    if ippan.underground_area is not None and len(str(ippan.underground_area)) > 0:
                        ws_ippan.cell(row=8+i, column=52).value = str(ippan.underground_area)
                        
                    if ippan.kasen_kaigan_name is not None and len(str(ippan.kasen_kaigan_name)) > 0:
                        ws_ippan.cell(row=8+i, column=53).value = str(ippan.kasen_kaigan_name)
                    
                    if ippan.crop_damage is not None and len(str(ippan.crop_damage)) > 0:
                        ws_ippan.cell(row=8+i, column=54).value = str(ippan.crop_damage)
                        
                    if ippan.weather_name is not None and len(str(ippan.weather_name)) > 0:
                        ws_ippan.cell(row=8+i, column=55).value = str(ippan.weather_name)
                        
                    if ippan.suigai_name is not None and len(str(ippan.suigai_name)) > 0:
                        ws_ippan.cell(row=8+i, column=56).value = str(ippan.suigai_name)
                    
                    ws_ippan.cell(row=8+i, column=57).value = ""
                        
                    if ippan.committed_at is not None and len(str(ippan.committed_at)) > 0:
                        ws_ippan.cell(row=8+i, column=58).value = str(ippan.committed_at)
                    
                    if ippan.upload_file_path is not None and len(str(ippan.upload_file_path)) > 0:
                        ws_ippan.cell(row=8+i, column=59).value = str(ippan.upload_file_path)

            ###################################################################
            ### EXCEL入出力処理(0100)
            ### ダウンロード用のEXCELファイルを保存する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken()関数 STEP 11/13.', 'DEBUG')
            ...
            ### トリガーデータを更新する。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 12/13.', 'DEBUG')
            ...
            try:
                ...
                print_log('[ERROR] P0900Action.action_D01_download_ippan_ken.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            ...
            ### 戻り値を戻す。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_D01_download_ippan_ken.handle()関数 STEP 13/13.', 'DEBUG')
            print_log('[INFO] P0900Action.action_D01_download_ippan_ken.handle()関数が正常終了しました。', 'INFO')
            return 0
        except:
            print_log('[ERROR] P0900Action.action_D01_download_ippan_ken.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            print_log('[ERROR] P0900Action.action_D01_download_ippan_ken.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_D01_download_ippan_ken.handle()関数が異常終了しました。', 'ERROR')
            return 8
