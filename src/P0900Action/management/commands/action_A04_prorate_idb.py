...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_A04_prorate_idb.py
### A05：按分処理
###############################################################################
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            ###################################################################
            reset_log()
            print_log('[INFO] P0900Action.action_A04_prorate_idb.handle()関数が開始しました。', 'INFO')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 STEP 1/5.', 'DEBUG')
            ...
            ###################################################################
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 STEP 2/5.', 'DEBUG')
            ...
            if trigger_list is None:
                print_log('[INFO] P0900Action.action_A04_prorate_idb.handle()関数が正常終了しました。', 'INFO')
                return 0
            ...
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].trigger_id = {}'.format(trigger_list[0].trigger_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].suigai_id = {}'.format(trigger_list[0].suigai_id), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].city_code = {}'.format(trigger_list[0].city_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].ken_code = {}'.format(trigger_list[0].ken_code), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].download_file_path = {}'.format(trigger_list[0].download_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].download_file_name = {}'.format(trigger_list[0].download_file_name), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].upload_file_path = {}'.format(trigger_list[0].upload_file_path), 'DEBUG')
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 trigger_list[0].upload_file_name = {}'.format(trigger_list[0].upload_file_name), 'DEBUG')

            ################################################################### 
            ### DBアクセス処理(0020)
            ### 当該トリガーの実行が終了したため、当該トリガーの状態、成功数、失敗数等を更新する。
            ################################################################### 
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 STEP 3/5.', 'DEBUG')
            ...
            try:
                ...
                ### 次のトリガーを発行する。
                ###############################################################
                print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 STEP 4/5.', 'DEBUG')
                connection_cursor.execute("""
                    INSERT INTO TRIGGER (
                ...
                print_log('[ERROR] P0900Action.action_A04_prorate_idb.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            ...
            ### 戻り値を戻す。
            ###################################################################
            print_log('[DEBUG] P0900Action.action_A04_prorate_idb.handle()関数 STEP 5/5.', 'DEBUG')
            print_log('[INFO] P0900Action.action_A04_prorate_idb.handle()関数が正常終了しました。', 'INFO')
            return 0
        except:
            print_log('[ERROR] P0900Action.action_A04_prorate_idb.handle()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
            print_log('[ERROR] P0900Action.action_A04_prorate_idb.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_A04_prorate_idb.handle()関数が異常終了しました。', 'ERROR')
            return 8
