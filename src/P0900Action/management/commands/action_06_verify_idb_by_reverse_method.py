...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_06_verify_idb_by_reverse_method.py
###############################################################################
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            ###################################################################
            print_log('[INFO] ########################################', 'INFO')
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数が開始しました。', 'INFO')
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 1/7.', 'INFO')
            ...
            ###################################################################
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 2/7.', 'INFO')
            ...
            if trigger_list is None:
                print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数が正常終了しました。', 'INFO')
                return 0
            ...
            ### ※ネストを浅くするために、処理対象外の場合、処理を終了させる。
            ###################################################################
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 3/7.', 'INFO')
            ...
            if ippan_reverse_list is None:
                print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数が警告終了しました。', 'INFO')
                return 4
            ...
            ###################################################################
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 4/7.', 'INFO')
            success_count = 0
            failure_count = 0
            epsilon = 0.0000001
            ...
            for ippan in ippan_reverse_list:
                ...
                ### 被害建物の延床面積(入力DB)から逆計算により被害建物棟数を求めた結果
                ###############################################################
                print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 4_1/7.', 'INFO')
                ...
                ### 被災世帯数(入力DB)から逆計算により被害建物棟数を求めた結果
                ###############################################################
                print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 4_2/7.', 'INFO')
                ...
                ### 被災事業所数(入力DB)から逆計算により被害建物棟数を求めた結果
                ###############################################################
                print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 4_3/7.', 'INFO')
            ...
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 5/7.', 'INFO')
            if failure_count == 0:
            ...
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 6/7.', 'INFO')
            if failure_count == 0:
                connection_cursor.execute("""
                    INSERT INTO TRIGGER (
            ...
            ### 戻り値セット処理(0090)
            ###################################################################
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数 STEP 7/7.', 'INFO')
            print_log('[INFO] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数が正常終了しました。', 'INFO')
            return 0
        
        except:
            print_log(sys.exc_info()[0], 'ERROR')
            print_log('[ERROR] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_06_verify_idb_by_reverse_method.handle()関数が異常終了しました。', 'ERROR')
            return 8
