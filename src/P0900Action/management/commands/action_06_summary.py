...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0900Action/management/commands/action_06_summary.py
###############################################################################
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            ###################################################################
            print_log('[INFO] ########################################', 'INFO')
            print_log('[INFO] P0900Action.action_06_summary.handle()関数が開始しました。', 'INFO')
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 1/10.', 'INFO')
            ...
            ###################################################################
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 2/10.', 'INFO')
            trigger_list = None
            ...
            if trigger_list is None:
                print_log('[INFO] P0900Action.action_06_summary.handle()関数が正常終了しました。', 'INFO')
                return 0
            ...
            ### ※ネストを浅くするために、処理対象外の場合、処理を終了させる。
            ###################################################################
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 3/10.', 'INFO')
            ...
            ### 集計データから集計データを1件削除する。
            ###################################################################
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 4/10.', 'INFO')
            ...
            ### 集計データに集計データを1件登録する。
            ###################################################################
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 5/10.', 'INFO')
            print_log('suigai_id_list[0] = {}'.format(suigai_id_list[0]), 'INFO')
            connection_cursor.execute("""
            ...
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 6/10.', 'INFO')
            ...
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 7/10.', 'INFO')
            print_log('suigai_id_list[0] = {}'.format(suigai_id_list[0]), 'INFO')
            ...
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 8/10.', 'INFO')
            print_log('repository_id_list[0] = {}'.format(repository_id_list[0]), 'INFO')
            connection_cursor.execute("""
            ...
            ### DBアクセス処理(0080)
            ################################################################### 
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 9/10.', 'INFO')
            transaction.commit()
            
            ###################################################################
            ### 戻り値セット処理(0090)
            ###################################################################
            print_log('[INFO] P0900Action.action_06_summary.handle()関数 STEP 10/10.', 'INFO')
            print_log('[INFO] P0900Action.action_06_summary.handle()関数が正常終了しました。', 'INFO')
            return 0
            ...
            print_log('[ERROR] P0900Action.action_06_summary.handle()関数でエラーが発生しました。', 'ERROR')
            print_log('[ERROR] P0900Action.action_06_summary.handle()関数が異常終了しました。', 'ERROR')
            return 8
