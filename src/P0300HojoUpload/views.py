...
# -*- coding: utf-8 -*-
###############################################################################
### ファイル名：P0300HojoUpload/views.py
### 公共土木施設補助事業調査票アップロード
###############################################################################
...
from .forms import HojoUploadForm
...
### コメントをセルにセットする。
###############################################################################
def add_comment(ws_hojo, ws_result, row, column, fill, com_id):
    ws_hojo.cell(row=row, column=column).fill = fill
    ws_result.cell(row=row, column=column).fill = fill
    
    msg = MESSAGE[com_id][3] + MESSAGE[com_id][4]

    if ws_hojo.cell(row=row, column=column).comment is None:
        ws_hojo.cell(row=row, column=column).comment = Comment(msg, '')
    else:
        ws_hojo.cell(row=row, column=column).comment = Comment(str(ws_hojo.cell(row=row, column=column).comment.text) + msg, '')
...
### template：P0300HojoUpload/index.html
### (1)GETの場合、公共土木施設補助事業調査票アップロード画面を表示する。
### (2)POSTの場合、アップロードされた公共土木施設補助事業調査票ファイルをチェックして、正常ケースの場合、DBに登録する。
### (3)POSTの場合、アップロードされた公共土木施設補助事業調査票ファイルをチェックして、警告ケースの場合、DBに登録する。
### ※複数EXCELシート未対応版
###############################################################################
...
def index_view(request):
    try:
        ...
        #######################################################################
        reset_log()
        print_log('[INFO] P0300HojoUpload.index_view()関数が開始しました。', 'INFO')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 request = {}'.format(request.method), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 1/35.', 'DEBUG')
        
        #######################################################################
        ### 局所変数セット処理(0010)
        ### チェック結果を格納するために局所変数をセットする。
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 2/35.', 'DEBUG')
        require_OK = []
        format_OK = []
        range_OK = []
        correlate_OK = []
        ...
        #######################################################################
        ### 条件分岐処理(0020)
        ### (1)GETの場合、公共土木施設補助事業調査票アップロード画面を表示して関数を抜ける。
        ### (2)POSTの場合、アップロードされた公共土木施設補助事業調査票ファイルをチェックする。
        ### ※関数の内部のネスト数を浅くするため。
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 3/35.', 'DEBUG')
        if request.method == 'GET':
            form = HojoUploadForm()
            return render(request, 'P0300HojoUpload/index.html', {'form': form})
        
        elif request.method == 'POST':
            form = HojoUploadForm(request.POST, request.FILES)
        ...
        ### ※関数の内部のネスト数を浅くするため。
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 4/35.', 'DEBUG')
        if form.is_valid():
            pass
        else:
        ...
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 5/35.', 'DEBUG')
        ...
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 upload_file_object = {}'.format(upload_file_object), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 upload_file_path = {}'.format(upload_file_path), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 upload_file_name = {}'.format(upload_file_name), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 output_file_path = {}'.format(output_file_path), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 output_file_name = {}'.format(output_file_name), 'DEBUG')
                
        #######################################################################
        ### EXCELファイル入出力処理(0050)
        ### (1)アップロードされた公共土木施設補助事業調査票ファイルのワークブックを読み込む。
        ...
        ### fill: 背景赤色の塗りつぶし
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 6/35.', 'DEBUG')
        ...
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 7/35.', 'DEBUG')
        max_row = []
        
        for ws_temp in ws_hojo:
            max_row_temp = 7
        ...
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 8/35.', 'DEBUG')
        ...
        ### (2)突合せチェック用のリストを生成する。
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 9/35.', 'DEBUG')
        ...
        #######################################################################
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 13/35.', 'DEBUG')
        if max_row[0] >= 8:
            for j in range(8, max_row[0] + 1):
                ### セル[8:2]: 水系・沿岸名に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=2).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=2, fill=fill, com_id=50)
                    require_NG.append([ws_hojo[0].title, j, 2, 50])
                else:
                    require_OK.append([ws_hojo[0].title, j, 2, 50])
                    
                ### セル[8:3]: 水系種別に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=3).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=3, fill=fill, com_id=51)
                    require_NG.append([ws_hojo[0].title, j, 3, 51])
                else:
                    require_OK.append([ws_hojo[0].title, j, 3, 51])
                    
                ### セル[8:4]: 河川・海岸名に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=4).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=4, fill=fill, com_id=52)
                    require_NG.append([ws_hojo[0].title, j, 4, 52])
                else:
                    require_OK.append([ws_hojo[0].title, j, 4, 52])
                    
                ### セル[8:5]: 河川種別に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=5).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=5, fill=fill, com_id=53)
                    require_NG.append([ws_hojo[0].title, j, 5, 53])
                else:
                    require_OK.append([ws_hojo[0].title, j, 5, 53])
                    
                ### セル[8:6]: 都道府県コードに値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=6).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=54)
                    require_NG.append([ws_hojo[0].title, j, 6, 54])
                else:
                    require_OK.append([ws_hojo[0].title, j, 6, 54])

                ### セル[8:7]: 工事番号に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=7).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=55)
                    require_NG.append([ws_hojo[0].title, j, 7, 55])
                else:
                    require_OK.append([ws_hojo[0].title, j, 7, 55])

                ### セル[8:8]: 枝番に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=8).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=56)
                    require_NG.append([ws_hojo[0].title, j, 8, 56])
                else:
                    require_OK.append([ws_hojo[0].title, j, 8, 56])

                ### セル[8:9]: 工事区分に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=9).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=57)
                    require_NG.append([ws_hojo[0].title, j, 9, 57])
                else:
                    require_OK.append([ws_hojo[0].title, j, 9, 57])

                ### セル[8:10]: 市区町村コードに値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=10).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=58)
                    require_NG.append([ws_hojo[0].title, j, 10, 58])
                else:
                    require_OK.append([ws_hojo[0].title, j, 10, 58])

                ### セル[8:11]: 工種区分に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=11).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=59)
                    require_NG.append([ws_hojo[0].title, j, 11, 59])
                else:
                    require_OK.append([ws_hojo[0].title, j, 11, 59])

                ### セル[8:12]: 
                ### セル[8:13]: 異常気象コードに値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=13).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=61)
                    require_NG.append([ws_hojo[0].title, j, 13, 61])
                else:
                    require_OK.append([ws_hojo[0].title, j, 13, 61])

                ### セル[8:14]: 水害発生月に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=14).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=62)
                    require_NG.append([ws_hojo[0].title, j, 14, 62])
                else:
                    require_OK.append([ws_hojo[0].title, j, 14, 62])

                ### セル[8:15]: 水害発生日に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=15).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=63)
                    require_NG.append([ws_hojo[0].title, j, 15, 63])
                else:
                    require_OK.append([ws_hojo[0].title, j, 15, 63])

                ### セル[8:16]: 決定額(千円)に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=16).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=64)
                    require_NG.append([ws_hojo[0].title, j, 16, 64])
                else:
                    require_OK.append([ws_hojo[0].title, j, 16, 64])

                ### セル[8:17]: 備考に値がセットされていることをチェックする。
                if ws_hojo[0].cell(row=j, column=17).value is None:
                    add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=65)
                    require_NG.append([ws_hojo[0].title, j, 17, 65])
                else:
                    require_OK.append([ws_hojo[0].title, j, 17, 65])
        ...
        #######################################################################
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 17/35.', 'DEBUG')
        if max_row[0] >= 8:
            for j in range(8, max_row[0] + 1):
                ...
                if ws_hojo[0].cell(row=j, column=2).value is None:
                    ...
                    if split_name_code(ws_hojo[0].cell(row=j, column=2).value)[-1].isdecimal() == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=2, fill=fill, com_id=150)
                        format_NG.append([ws_hojo[0].title, j, 2, 150])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 2, 150])
                    
                ### セル[8:3]: 水系種別について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=3).value is None:
                    pass
                else:
                    if split_name_code(ws_hojo[0].cell(row=j, column=3).value)[-1].isdecimal() == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=3, fill=fill, com_id=151)
                        format_NG.append([ws_hojo[0].title, j, 3, 151])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 3, 151])
                    
                ### セル[8:4]: 河川・海岸名について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=4).value is None:
                    pass
                else:
                    if split_name_code(ws_hojo[0].cell(row=j, column=4).value)[-1].isdecimal() == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=4, fill=fill, com_id=152)
                        format_NG.append([ws_hojo[0].title, j, 4, 152])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 4, 152])
                    
                ### セル[8:5]: 河川種別について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=5).value is None:
                    pass
                else:
                    if split_name_code(ws_hojo[0].cell(row=j, column=5).value)[-1].isdecimal() == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=5, fill=fill, com_id=153)
                        format_NG.append([ws_hojo[0].title, j, 5, 153])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 5, 153])
                    
                ### セル[8:6]: 都道府県コードについて形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=6).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=6).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=6).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=154)
                        format_NG.append([ws_hojo[0].title, j, 6, 154])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 6, 154])
                    
                ### セル[8:7]: 工事番号について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=7).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=7).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=7).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=155)
                        format_NG.append([ws_hojo[0].title, j, 7, 155])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 7, 155])
                    
                ### セル[8:8]: 枝番について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=8).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=8).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=8).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=156)
                        format_NG.append([ws_hojo[0].title, j, 8, 156])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 8, 156])
                    
                ### セル[8:9]: 工事区分について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=9).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=9).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=9).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=157)
                        format_NG.append([ws_hojo[0].title, j, 9, 157])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 9, 157])
                    
                ### セル[8:10]: 市区町村コードについて形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=10).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=10).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=10).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=158)
                        format_NG.append([ws_hojo[0].title, j, 10, 158])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 10, 158])
                    
                ### セル[8:11]: 工種区分について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=11).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=11).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=11).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=159)
                        format_NG.append([ws_hojo[0].title, j, 11, 159])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 11, 159])
                    
                ### セル[8:12]: 
                ### セル[8:13]: 異常気象コードについて形式が正しいことをチェックする。
                if ws_hojo[i].cell(row=j, column=13).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=13).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=13).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=161)
                        format_NG.append([ws_hojo[0].title, j, 13, 161])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 13, 161])
                    
                ### セル[8:14]: 水害発生月について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=14).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=14).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=14).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=162)
                        format_NG.append([ws_hojo[0].title, j, 14, 162])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 14, 162])
                    
                ### セル[8:15]: 水害発生日について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=15).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=15).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=15).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=163)
                        format_NG.append([ws_hojo[0].title, j, 15, 163])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 15, 163])
                    
                ### セル[8:16]: 決定額(千円)について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=16).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=16).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=16).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=164)
                        format_NG.append([ws_hojo[0].title, j, 16, 164])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 16, 164])
                    
                ### セル[8:17]: 備考について形式が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=17).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=17).value, int) == False and \
                        isinstance(ws_hojo[0].cell(row=j, column=17).value, float) == False:
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=165)
                        format_NG.append([ws_hojo[0].title, j, 17, 165])
                    else:
                        format_OK.append([ws_hojo[0].title, j, 17, 165])
        ...
        #######################################################################
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 21/35.', 'DEBUG')
        if max_row[0] >= 8:
            for j in range(8, max_row[0] + 1):
                ...
                if ws_hojo[0].cell(row=j, column=6).value is None:
                    ...
                    if isinstance(ws_hojo[0].cell(row=j, column=6).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=6).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=6).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=6, fill=fill, com_id=254)
                            range_NG.append([ws_hojo[0].title, j, 6, 254])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 6, 254])

                ### セル[8:7]: 工事番号について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=7).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=7).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=7).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=7).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=7, fill=fill, com_id=255)
                            range_NG.append([ws_hojo[0].title, j, 7, 255])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 7, 255])
                
                ### セル[8:8]: 枝番について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=8).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=8).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=8).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=8).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=8, fill=fill, com_id=256)
                            range_NG.append([ws_hojo[0].title, j, 8, 256])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 8, 256])

                ### セル[8:9]: 工事区分について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=9).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=9).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=9).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=9).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=9, fill=fill, com_id=257)
                            range_NG.append([ws_hojo[0].title, j, 9, 257])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 9, 257])

                ### セル[8:10]: 市区町村コードについて範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=10).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=10).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=10).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=10).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=10, fill=fill, com_id=258)
                            range_NG.append([ws_hojo[0].title, j, 10, 258])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 10, 258])

                ### セル[8:11]: 工種区分について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=11).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=11).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=11).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=11).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=11, fill=fill, com_id=259)
                            range_NG.append([ws_hojo[0].title, j, 11, 259])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 11, 259])
                ...
                if ws_hojo[0].cell(row=j, column=13).value is None:
                    ...
                    if isinstance(ws_hojo[0].cell(row=j, column=13).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=13).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=13).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=13, fill=fill, com_id=261)
                            range_NG.append([ws_hojo[0].title, j, 13, 261])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 13, 261])
                
                ### セル[8:14]: 水害発生月について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=14).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=14).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=14).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=14).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=14, fill=fill, com_id=262)
                            range_NG.append([ws_hojo[0].title, j, 14, 262])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 14, 262])

                ### セル[8:15]: 水害発生日について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=15).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=15).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=15).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=15).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=15, fill=fill, com_id=263)
                            range_NG.append([ws_hojo[0].title, j, 15, 263])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 15, 263])

                ### セル[8:16]: 決定額(千円)について範囲が正しいことをチェックする。
                if ws_hojo[i].cell(row=j, column=16).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=16).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=16).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=16).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=16, fill=fill, com_id=264)
                            range_NG.append([ws_hojo[0].title, j, 16, 264])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 16, 264])

                ### セル[8:17]: 備考について範囲が正しいことをチェックする。
                if ws_hojo[0].cell(row=j, column=17).value is None:
                    pass
                else:
                    if isinstance(ws_hojo[0].cell(row=j, column=17).value, int) == True or \
                        isinstance(ws_hojo[0].cell(row=j, column=17).value, float) == True:
                        if float(ws_hojo[0].cell(row=j, column=17).value) < 0:
                            add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=17, fill=fill, com_id=265)
                            range_NG.append([ws_hojo[0].title, j, 17, 265])
                        else:
                            range_OK.append([ws_hojo[0].title, j, 17, 265])
        ...
        #######################################################################
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 25/35.', 'DEBUG')
        if max_row[0] >= 8:
        ...
        #######################################################################
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 29/35.', 'DEBUG')
        if max_row[0] >= 8:
            for j in range(8, max_row[i] + 1):
                ...
                if ws_hojo[0].cell(row=j, column=2).value is None:
                    ...
                    if ws_hojo[0].cell(row=j, column=2).value not in list(building_code_list) and \
                        ws_hojo[0].cell(row=j, column=2).value not in list(building_name_list) and \
                        ws_hojo[0].cell(row=j, column=2).value not in list(building_name_code_list):
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=2, fill=fill, com_id=450)
                        compare_NG.append([ws_hojo[0].title, j, 2, 450])
                    else:
                        compare_OK.append([ws_hojo[0].title, j, 2, 450])
                
                ### セル[8:3]: 水系種別についてデータベースに登録されている値と突合せチェックする。
                if ws_hojo[0].cell(row=j, column=4).value is None:
                    pass
                else:
                    if ws_hojo[0].cell(row=j, column=3).value not in list(building_code_list) and \
                        ws_hojo[0].cell(row=j, column=3).value not in list(building_name_list) and \
                        ws_hojo[0].cell(row=j, column=3).value not in list(building_name_code_list):
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=3, fill=fill, com_id=451)
                        compare_NG.append([ws_hojo[0].title, j, 3, 451])
                    else:
                        compare_OK.append([ws_hojo[0].title, j, 3, 451])
                    
                ### セル[8:4]: 河川・海岸名についてデータベースに登録されている値と突合せチェックする。
                if ws_hojo[0].cell(row=j, column=4).value is None:
                    pass
                else:
                    if ws_hojo[0].cell(row=j, column=4).value not in list(underground_code_list) and \
                        ws_hojo[0].cell(row=j, column=4).value not in list(underground_name_list) and \
                        ws_hojo[0].cell(row=j, column=4).value not in list(underground_name_code_list):
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=4, fill=fill, com_id=452)
                        compare_NG.append([ws_hojo[0].title, j, 4, 452])
                    else:
                        compare_OK.append([ws_hojo[0].title, j, 4, 452])
                    
                ### セル[8:5]: 河川種別についてデータベースに登録されている値と突合せチェックする。
                if ws_hojo[0].cell(row=j, column=5).value is None:
                    pass
                else:
                    if ws_hojo[0].cell(row=j, column=5).value not in list(flood_sediment_code_list) and \
                        ws_hojo[0].cell(row=j, column=5).value not in list(flood_sediment_name_list) and \
                        ws_hojo[0].cell(row=j, column=5).value not in list(flood_sediment_name_code_list):
                        add_comment(ws_hojo=ws_hojo[0], ws_result=ws_result[0], row=j, column=5, fill=fill, com_id=453)
                        compare_NG.append([ws_hojo[0].title, j, 5, 453])
                    else:
                        compare_OK.append([ws_hojo[0].title, j, 5, 453])
        ...
        ### チェック結果ファイルを保存する。
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 30/35.', 'DEBUG')
        wb.save(output_file_path)

        #######################################################################
        ### ログ出力処理(7000)
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 31/35.', 'DEBUG')
        if len(require_NG) > 0 or \
            len(format_NG) > 0 or \
            len(range_NG) > 0 or \
            len(correlate_NG) > 0 or \
            len(compare_NG) > 0:
            print_log('[DEBUG] P0300HojoUpload.index_view()関数 False', 'DEBUG')
        else:
            print_log('[DEBUG] P0300HojoUpload.index_view()関数 True', 'DEBUG')
            
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 max_row = {}'.format(max_row), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(require_NG) = {}'.format(len(require_NG)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(format_NG) = {}'.format(len(format_NG)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(range_NG) = {}'.format(len(range_NG)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(correlate_NG) = {}'.format(len(correlate_NG)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(compare_NG) = {}'.format(len(compare_NG)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(require_OK) = {}'.format(len(require_OK)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(format_OK) = {}'.format(len(format_OK)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(range_OK) = {}'.format(len(range_OK)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(correlate_OK) = {}'.format(len(correlate_OK)), 'DEBUG')
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 len(compare_OK) = {}'.format(len(compare_OK)), 'DEBUG')
        ...
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 32/35.', 'DEBUG')
        if len(require_NG) > 0 or \
            len(format_NG) > 0 or \
            len(range_NG) > 0 or \
            len(correlate_NG) > 0 or \
            ...
            try:
                ...
                ### 市区町村をアップロード処理の単位とする。
                ### 当該市区町村の全データを入れ替える。
                ### 既存の市区町村のHOJO_FILE、HOJO、HOJO_SUMMARYのデータは、削除日時をセットして、削除済の扱いとする。
                ### ※入力チェックでエラーが発見された場合、
                ###############################################################
                connection_cursor.execute("""
                    ...
                    """, [
                        split_name_code(ws_hojo[0].cell(row=7, column=3).value)[-1], 
                    ])
                ...
                connection_cursor.execute("""
                    INSERT INTO TRIGGER (
                        ...
                        convert_empty_to_none(split_name_code(ws_hojo[0].cell(row=7, column=2).value)[-1]), ### ken_code 
                        convert_empty_to_none(split_name_code(ws_hojo[0].cell(row=7, column=3).value)[-1]), ### city_code 
                ...
                print_log('[ERROR] P0300HojoUpload.index_view()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
                ### connection_cursor.rollback()
                connection_cursor.execute("""ROLLBACK""", [])
            finally:
                connection_cursor.close()
                
            ### src/P0300HojoUpload/templates/P0300HojoUpload/fail.htmlを使用する。
            template = loader.get_template('P0300HojoUpload/fail.html')
            context = {
            ...
            print_log('[INFO] P0300HojoUpload.index_view()関数が正常終了しました。', 'INFO')
        ...
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 33/35.', 'DEBUG')
        connection_cursor = connection.cursor()
        try:
            connection_cursor.execute("""BEGIN""", [])
            ...
            ### ※入力チェックでエラーが発見されなかった場合、
            ###################################################################
            for i, _ in enumerate(ws_hojo):
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 34_1/35.', 'DEBUG')
                ### suigai_id__max で正しい。
                ...
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 suigai_id = {}'.format(suigai_id), 'DEBUG')
                ...
                ### ※入力チェックでエラーが発見されなかった場合、
                ###############################################################
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 34_2/35.', 'DEBUG')
                connection_cursor.execute("""
                    INSERT INTO SUIGAI (
                        ...
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=2).value)[-1]), ### ken_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=3).value)[-1]), ### city_code 
                        convert_empty_to_none(ws_hojo[i].cell(row=7, column=4).value), ### begin_date 
                        convert_empty_to_none(ws_hojo[i].cell(row=7, column=5).value), ### end_date 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=6).value)[-1]), ### cause_1_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=7).value)[-1]), ### cause_2_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=8).value)[-1]), ### cause_3_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=7, column=9).value)[-1]), ### area_id 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=10, column=2).value)[-1]), ### suikei_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=10, column=4).value)[-1]), ### kasen_code 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=10, column=6).value)[-1]), ### gradient_code 
                        convert_empty_to_none(ws_hojo[i].cell(row=14, column=2).value), ### residential_area 
                        convert_empty_to_none(ws_hojo[i].cell(row=14, column=3).value), ### agricultural_area 
                        convert_empty_to_none(ws_hojo[i].cell(row=14, column=4).value), ### underground_area 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=14, column=6).value)[-1]), ### kasen_kaigan_code 
                        convert_empty_to_none(ws_hojo[i].cell(row=14, column=8).value), ### crop_damaga 
                        convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=14, column=10).value)[-1]), ### weather_id 
                        None, ### deleted_at 
                        upload_file_path, ### upload_file_path 
                        upload_file_name, ### upload_file_name 
                ...
                ### ※入力チェックでエラーが発見されなかった場合、
                ###############################################################
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 34_3/35.', 'DEBUG')
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 max_row[i] = {}'.format(max_row[i]), 'DEBUG')
                if max_row[i] >= 20:
                    for j in range(20, max_row[i] + 1):
                        print_log('[DEBUG] P0300HojoUpload.index_view()関数 j = {}'.format(j), 'DEBUG')
                        connection_cursor.execute(""" 
                            INSERT INTO IPPAN (
                                ...
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=2).value), ### ippan_name
                                suigai_id, ### suigai_id
                                convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=j, column=3).value)[-1]), ### building_code
                                convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=j, column=4).value)[-1]), ### underground_code
                                convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=j, column=5).value)[-1]), ### flood_sediment_code
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=6).value), ### building_lv00
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=7).value), ### building_lv01_49
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=8).value), ### building_lv50_99
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=9).value), ### building_lv100
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=10).value), ### building_half
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=11).value), ### building_full
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=12).value), ### floor_area
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=13).value), ### family
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=14).value), ### office
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=15).value), ### farmer_fisher_lv00
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=16).value), ### farmer_fisher_lv01_49
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=17).value), ### farmer_fisher_lv50_99
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=18).value), ### farmer_fisher_lv100
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=19).value), ### farmer_fisher_full
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=20).value), ### employee_lv00
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=21).value), ### employee_lv01_49
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=22).value), ### employee_lv50_99
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=23).value), ### employee_lv100
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=24).value), ### employee_full
                                convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=j, column=25).value)[-1]), ### industry_code
                                convert_empty_to_none(split_name_code(ws_hojo[i].cell(row=j, column=26).value)[-1]), ### usage_code
                                convert_empty_to_none(ws_hojo[i].cell(row=j, column=27).value), ### comment
                                None, ### deleted_at 
                            ])
                ...
                ### ※入力チェックでエラーが発見されなかった場合、
                ###############################################################
                print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 34_5/35.', 'DEBUG')
        ...
        ### ※入力チェックでエラーが発見されなかった場合、
        #######################################################################
        print_log('[DEBUG] P0300HojoUpload.index_view()関数 STEP 35/35.', 'DEBUG')
        ### src/P0300HojoUpload/templates/P0300HojoUpload/success.htmlを使用する。
        ### 上記はテンプレートファイルの場所がわからなくなることがあるためのメモである。
        template = loader.get_template('P0300HojoUpload/success.html')
        context = {}
        print_log('[INFO] P0300HojoUpload.index_view()関数が正常終了しました。', 'INFO')
        return HttpResponse(template.render(context, request))
        ...
        print_log('[ERROR] P0300HojoUpload.index_view()関数 {}'.format(sys.exc_info()[0]), 'ERROR')
        print_log('[ERROR] P0300HojoUpload.index_view()関数でエラーが発生しました。', 'ERROR')
        print_log('[ERROR] P0300HojoUpload.index_viwe()関数が異常終了しました。', 'ERROR')
